...
namespace {
	...
	void unlinkPath(const char *path) {
		...
		fillName(request.path, sizeof(request.path), request.pathLength, path);
		vfsRequest(VFS_UNLINK_MSG_TYPE, request, reply);
	}
	...
	auto testMountRefresh() -> bool {
		...
		if (!vfsRequest(VFS_MOUNT_REFRESH_MSG_TYPE, request, reply)) {
			return fail("mount refresh", "request failed");
		}

		return expectOk("mount refresh", reply.success and reply.status == VFS_STATUS_OK and reply.volumeCount >= 2, "VFS did not report reserved volumes");
	}
	...
	auto mountedPartitionVolumes(VolumeList &out) -> bool {
		char text[VFS_MAX_READ_SIZE + 1] {};
		uint32_t textLength {};

		if (!readPath("Devices:/volumes", text, sizeof(text), textLength)) {
			return false;
		}
	...
	auto testPartitionRootFileLinks(const char *volume) -> bool {
		...
		if (!createFile(base)) {
			printf("[VFS test] FAIL partition %s root file create\r\n", volume);
			fflush(stdout);
			return false;
		}

		if (!writePath(base, payload)) {
			printf("[VFS test] FAIL partition %s root file write\r\n", volume);
			fflush(stdout);
			return false;
		}
		...
		if (!readPath(base, readBack, sizeof(readBack), readBackLength) or strcmp(readBack, payload) != 0) {
			printf("[VFS test] FAIL partition %s root file readback\r\n", volume);
			fflush(stdout);
			return false;
		}

		if (!mkdirPath(dir)) {
			printf("[VFS test] FAIL partition %s mkdir\r\n", volume);
			fflush(stdout);
			return false;
		}

		if (!symlinkPath(symlinkTarget, symlink)) {
			printf("[VFS test] FAIL partition %s symlink create\r\n", volume);
			fflush(stdout);
			return false;
		}
		...
		if (!readLinkPath(symlink, linkTarget, sizeof(linkTarget)) or strcmp(linkTarget, symlinkTarget) != 0) {
			printf("[VFS test] FAIL partition %s symlink verify\r\n", volume);
			fflush(stdout);
			return false;
		}

		if (!hardLinkPath(base, hardLink)) {
			printf("[VFS test] FAIL partition %s hard link create\r\n", volume);
			fflush(stdout);
			return false;
		}
		...
		if (!readPath(hardLink, hardReadBack, sizeof(hardReadBack), hardReadBackLength) or strcmp(hardReadBack, payload) != 0) {
			printf("[VFS test] FAIL partition %s hard link readback\r\n", volume);
			fflush(stdout);
			return false;
		}
		...
		printf("[VFS test] PASS partition %s root file/link workflow\r\n", volume);
		fflush(stdout);
