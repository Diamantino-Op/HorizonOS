...
namespace {
	...
	auto registerWithNameRegistry(const char *name) -> bool {
		...
		if (send_horizonos_message(fat32Port, 1, &msg) != 0) {
			return false;
		}
		...
		const int ret = receive_horizonos_message(fat32Port, &replyMsg, &filter);
	...
	auto registerFsHandler() -> bool {
		...
		if (send_horizonos_message(fat32Port, storagePort, &msg) != 0) {
			return false;
		}
	...
	auto readDevicePage(const uint64_t deviceId, const uint64_t lba, uint64_t &phys, uint64_t &virt) -> bool {
		...
		do {
			ret = receive_horizonos_message(storageReplyPort, &recv, &filter);
	...
	void sendReply(const uint64_t type, const hos_msg &request, Reply &reply) {
		...
		send_horizonos_message(fat32Port, request.src_port, &replyMsg);
	}
	...
	[[noreturn]] auto readOnlyHandler(const uint64_t requestType, const uint64_t replyType, const uint32_t status) -> void * {
		...
		for (;;) {
			...
			if (receive_horizonos_message(fat32Port, &msg, &filter) != 0) {
				continue;
			}
		...
		for (;;) {
			...
			if (data.deviceId != 0 and data.blockCount != 0 and data.blockSize != 0 and validName(data.deviceName, data.deviceNameLength, sizeof(data.deviceName), deviceName)) {
				...
				if (volume.load()) {
					...
					mounts.push_back(MountedFat32 { .mountId = mountId, .device = device });
					reply.success = true;
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
	if (register_horizonos_port(reinterpret_cast<long *>(&fat32Port)) != 0) {
