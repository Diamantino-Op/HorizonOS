...
public:
	...
	static auto setupScratchpads(ControllerMemory &memory, uint32_t maxScratchpads) -> bool;
	static auto setupCommandRing(ControllerMemory &memory, uint64_t operationalBase) -> bool;
	static auto setupEventRing(ControllerMemory &memory, uint64_t runtimeBase) -> bool;
	...
	static auto waitForPortReset(const MappedController &controller, uint8_t port) -> bool;
	...
	static auto setupTransferRing(XhciDevice &device) -> bool;
	static auto setupEndpointTransferRing(UsbEndpoint &endpoint) -> bool;
