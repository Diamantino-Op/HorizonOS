...
private:
	auto readBlock(uint64_t fsBlock, uint8_t *buffer) -> bool;
	auto writeBlock(uint64_t fsBlock, const uint8_t *buffer) -> bool;
	auto readIndirectPointer(uint32_t block, uint64_t index, uint64_t &fsBlock) -> bool;
	auto readDoubleIndirectPointer(uint32_t block, uint64_t index, uint64_t &fsBlock) -> bool;
	auto readTripleIndirectPointer(uint32_t block, uint64_t index, uint64_t &fsBlock) -> bool;
